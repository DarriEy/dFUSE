...
static int sundials_rhs(realtype t, N_Vector y, N_Vector ydot, void* user_data) {
    ...
    // Compute derivatives to float buffer
    ...
    // Manually set SWE derivative (since compute_derivatives skips it)
    if (data->config->enable_snow) {
        // SWE is the last state variable
        int swe_idx = data->n_states - 1;
        // dSWE/dt = snowfall rate - melt rate
        // forcing->precip is total precip, flux.rain is liquid
        // So snowfall = forcing->precip - flux.rain (approximately) or calculated in compute_snow
        
        // Accurate calculation based on compute_snow logic:
        // In compute_snow: SWE_new = SWE + snow - melt
        // So dSWE/dt ≈ (SWE_new - state.SWE) / dt (where dt=1.0 for rates in fuse usually)
        // Since SUNDIALS integrates rates, we can approximate:
        ydot_arr[swe_idx] = static_cast<realtype>(SWE_new - state.SWE);
    }
